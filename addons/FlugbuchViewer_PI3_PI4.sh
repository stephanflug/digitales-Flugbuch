...
# === 0. PrÃ¼fe, ob das ein Pi Zero oder Zero 2 ist ===
IS_ZERO=0
...
# 1. Pakete fÃ¼r Kiosk-Modus und Splashscreen installieren
sudo apt update
sudo apt install --no-install-recommends -y \
  xserver-xorg x11-xserver-utils xinit openbox chromium-browser unclutter fbi \
  lighttpd

# 2. Autologin fÃ¼r flugbuch auf tty1 einrichten
...
# 3. Watchdog-Skript fÃ¼r Chromium-Kiosk anlegen (mit HTTP-Check!)
...
while true; do
  ...
  # PrÃ¼fe alle 10 Sekunden, ob die Seite erreichbar ist
...
# 3b. Logfile fÃ¼r Watchdog anlegen (Rechte setzen, falls noch nicht vorhanden)
...
# 4. .xinitrc fÃ¼r Splash-Logo + Watchdog anlegen
...
# 5. .bash_profile fÃ¼r Autostart auf tty1 (NICHT bei SSH)
...
# 6. CGI-Skript fÃ¼r Web-URL-Ã„nderung installieren
...
if grep -qi "Zero" /proc/device-tree/model 2>/dev/null; then
  ...
  echo "data: Dieses Kiosk-Setup ist auf Pi Zero/Zero2 nicht unterstÃ¼tzt und wurde nicht installiert."
  echo "data: Bitte verwende einen leistungsfÃ¤higeren Pi (z.B. 3, 4, 5)."
  exit 0
fi
...
echo "data: Kiosk-URL gesetzt. Nach dem nÃ¤chsten Reboot startet Chromium mit dieser URL."
echo ""
EOF
sudo chmod +x "$CGI"

# 7. CGI-Skript fÃ¼r Loganzeige
...
# 10. Button in index.html einfÃ¼gen
...
if ! sudo grep -q "set_kiosk_url.html" "$INDEX_HTML"; then
  echo "FÃ¼ge Kiosk-Modus-Button zur index.html hinzu..."
...
echo "Ã–ffne im Browser: http://<IP>/set_kiosk_url.html"
